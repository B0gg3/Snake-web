...
// set basic variables for the snake and score.
export let snakeMovement = 5
let body = [{ x: 21, y: 21 }]
let newPiece = 0
let score = 0

export function updateSnake() { // update the snakes body and add pieces.
    addPieceses()
...
function snakeSpeedIncrease() { // increase speed of snake after every third point
    if (score % 3 === 0) {
        snakeMovement += 2
    }
}

export function getScore() { // get score
    return score
}

export function clearSnake() { // clear snake, to be able to restart the game.
    body = [{ x: 21, y: 21 }]
    score = 0
    snakeMovement = 4
}
